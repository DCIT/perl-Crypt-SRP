...
for (1..3) {
  my $I = "user$_";
  my $P = "secret$_";
  my ($s, $v) = $cli->compute_verifier_and_salt($I, $P, 32, $fmt);
...
post '/auth/srp_step1' => sub {
    ...
    if ($USERS{$I} && $USERS{$I}->{salt} && $USERS{$I}->{verifier}) {
      ...
      $dump = undef;
      my ($B, $s) = $srv->server_fake_B_s($I, 32, 'any nonce', $fmt);
      return $self->render_json({B=>$B, s=>$s, token=>$token});
    }
  };
...
post '/auth/srp_step2' => sub {
    ...
    my $srv = Crypt::SRP->new('RFC5054-1024bit', 'SHA1')->load($TOKENS{$token}); #restore state

    return $self->render_json({status=>'error'}) unless $srv->server_verify_M1($M1, $fmt);
